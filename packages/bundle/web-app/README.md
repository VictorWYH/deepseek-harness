...
[`dashboard.patch.yml`](dashboard.patch.yml) is an optional second patch layer for the same bundle: it inserts the Agent-first Dashboard shell ([`dsh-client-ui-dashboard-shell`](../../client/ui-dashboard-shell/README.md)) and disables the native sidebar and other shipped chrome while keeping `ui-conversation` and the services it depends on. The shipped `cordis.patch.yml` stays native: apply the overlay with `dsh web --patch <path-to-dashboard.patch.yml>`, or mount it through the web e2e scaffold's `extraOverlayPath` (see `apps/web/tests/dashboard-shell.e2e.ts`).
