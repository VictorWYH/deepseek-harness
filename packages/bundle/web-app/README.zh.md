...
## Dashboard 壳层 overlay

[`dashboard.patch.yml`](dashboard.patch.yml) 是同一组合包的可选第二 patch 层：它插入 Agent 优先的 Dashboard 壳（[`dsh-client-ui-dashboard-shell`](../../client/ui-dashboard-shell/README.zh.md)），并禁用原生侧栏及其余随附 chrome，同时保留 `ui-conversation` 及其依赖的服务。随附的 `cordis.patch.yml` 保持原生不变：使用 `dsh web --patch <path-to-dashboard.patch.yml>` 应用该 overlay，或通过 web e2e scaffold 的 `extraOverlayPath` 挂载（见 `apps/web/tests/dashboard-shell.e2e.ts`）。
