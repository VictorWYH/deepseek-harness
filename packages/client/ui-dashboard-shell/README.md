...
Product shell plugin (phase 1 of the Agent-native Dashboard): a dark two-column product frame registered into the runtime's built-in `root` slot at priority `-1` — the lowest rank renders, so this entry shadows the native AppFrame's priority-0 registration and takes over the whole browser surface. The [web-app bundle](../../bundle/web-app/cordis.patch.yml) disables the native chrome (ui-layout / ui-sidebar / ui-conversation and every feature that occupied their seats) and keeps the low-level services (client-runtime, ui-renderer, ui-theme, locale, ui-settings) plus this shell.
